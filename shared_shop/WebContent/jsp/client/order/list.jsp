	...
	<div class="container side_wrap">
		...
		<article class="main">
			...
			<div class="list">
				...
				<tr>
					<td>
						<a href="<%=request.getContextPath()%>/order/detail?id=${orderSum.order_id}">
							<c:out value="${orderSum.date}" />
					</td>
					<td>
						
						<c:choose>
						<c:when test="${orderSum.pay_method==1}">
						クレジットカード
						</c:when>
						<c:when test="${orderSum.pay_method==2}">
						銀行振り込み
						</c:when>
						<c:when test="${orderSum.pay_method==3}">
						着払い
						</c:when>
						<c:when test="${orderSum.pay_method==4}">
						電子マネー
						</c:when>
						<c:when test="${orderSum.pay_method==5}">
						コンビニ決済
						</c:when>
