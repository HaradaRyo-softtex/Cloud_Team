...
<body  class="admin user_update_input">
	...
	<div class="container side_wrap">
		...
		<article class="main">
			...
			<form method="post" action="<%=request.getContextPath()%>/admin/user/update/input" class="user_info_form">
					<br />
					<ul class="input_list">
						<li>
							<label>
						...
						<li>
							...
							<c:choose>
								...
								<c:when test="${userForm.authority==Constant.AUTH_ADMIN}">
									<input type="radio" name="authority" value="<%=Constant.AUTH_SYSTEM%>"/>
									<%=Constant.AUTH_SYSTEM_STR%>
									<input type="radio" name="authority" value="<%=Constant.AUTH_ADMIN%>"  checked="checked"/>
									<%=Constant.AUTH_ADMIN_STR%>
								</c:when>	
								<c:when test="${userForm.authority==Constant.AUTH_SYSTEM}">
									...
									<%=Constant.AUTH_SYSTEM_STR%>
									<input type="radio" name="authority" value="<%=Constant.AUTH_ADMIN%>"  />
									<%=Constant.AUTH_ADMIN_STR%>
								</c:when>	
							...
							<span class="input_value">
								<c:choose>
								<c:when test="${userForm.authority==Constant.AUTH_ADMIN}">
									<input type="hidden" name="authority" value="<%=Constant.AUTH_ADMIN%>"/>
									<%=Constant.AUTH_ADMIN_STR%>
								</c:when>	
								<c:when test="${userForm.authority==Constant.AUTH_SYSTEM}">
									<input type="hidden" name="authority" value="<%=Constant.AUTH_SYSTEM%>"/>
									<%=Constant.AUTH_SYSTEM_STR%>
								</c:when>	
								<c:otherwise><%--選択されている権限がシステム管理者、運用管理者以外の場合 --%>
									<input type="hidden" name="authority" value="<%=Constant.AUTH_CLIENT%>"/>
				...
				<form action="<%=request.getContextPath()%>/admin/user/detail">
					<input type="hidden" name="id" value="${userForm.id}"/>
