...
/**
 * 会員情報登録入力画面用コントローラ（管理者用）
 * 
 * @author System Shared.co.,Ltd.
 */
@WebServlet("/admin/user/regist/input")
public class UserRegistInputAdminController extends HttpServlet {
	/** シリアルID */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 会員一覧画面で、「新規会員登録」ボタンを押された時、会員登録画面を表示する
		HttpSession session = request.getSession();
		session.removeAttribute("userForm");
		request.getRequestDispatcher("/jsp/admin/user/regist_input.jsp").forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (backflg != null && backflg.equals(Constant.BACKFLG_OFF)) {
			...
			if (errorMessageList.size() > 0) {
				...
				response.sendRedirect(request.getContextPath() + "/admin/user/regist/confirm");
			}
