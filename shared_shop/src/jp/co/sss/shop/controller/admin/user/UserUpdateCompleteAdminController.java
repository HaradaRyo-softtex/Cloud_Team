...
/**
 * 会員情報変更完了画面用コントローラ（管理者用）
 * 
 * @author System Shared.co.,Ltd.
 */
@WebServlet("/admin/user/update/complete")
public class UserUpdateCompleteAdminController extends HttpServlet {
	/** シリアルID */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 1つ前のURL情報を取得しチェック
		if (URLValid.isNOTCorrectReferer(request.getHeader("REFERER"), URLConstant.URL_ADMIN_USER_UPDATE_CONFIRM)) {
			// 遷移元が確認画面以外の場合エラーとする
		...
		if (user == null) {
			...
			request.getRequestDispatcher("/jsp/admin/user/update_complete.jsp").forward(request, response);
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		response.sendRedirect(request.getContextPath() + "/admin/user/update/complete?id=" + user.getId());
	}
